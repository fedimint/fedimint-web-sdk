...
import {
  createFaucetInvoice,
  getFaucetGatewayInfo,
  getRequestCounter,
} from '../test/testUtils'
import { payFaucetInvoice } from '../test/testUtils'
...
walletTest(
  ...
  async ({ wallet }) => {
    ...
    const counterBefore = getRequestCounter(wallet)
    ...
    expect(getRequestCounter(wallet)).toBe(counterBefore + 3)
...
walletTest(
  ...
  async ({ wallet }) => {
    ...
    expect(getRequestCounter(wallet)).toBe(counterBefore + 1)
...
walletTest(
  ...
  async ({ fundedWallet }) => {
    ...
    const externalInvoice = await createFaucetInvoice(1)
    const gatewayInfo = await getFaucetGatewayInfo(fundedWallet)
...
walletTest(
  ...
  async ({ wallet }) => {
    ...
    const gatewayInfo = await getFaucetGatewayInfo(wallet)
    ...
    await expect(payFaucetInvoice(invoice.invoice)).resolves.toBeDefined()
